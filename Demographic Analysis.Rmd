...
unmerged <- unmerged %>%
  ...
  slice_head(n = 1) %>%        # keep the earlier of those 2 → 2nd most recent
  ungroup()
...
merged_subj <- merged %>%
  ...
  summarise(
    ...
    sex = first(sex),           # categorical — take first since it doesn’t change
    fhr = first(fhr),
    group = first(group),
    race = first(race),
    .groups = "drop"
...
# Step 4: Combine mean ± SD with paste
demo_all <- demo_all %>%
  mutate(
    `Age (M ± SD)` = sprintf("%.2f (%.2f)", age_mean, age_sd),
  ...
  select(group_label, n, `Age (M ± SD)`, `Female N (%)`, `White N`, `Black N`, `Asian N`, `First Nation N`, `Other N`)
