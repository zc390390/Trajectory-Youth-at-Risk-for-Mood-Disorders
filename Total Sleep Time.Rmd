...
nonlinear_order <- gamm(
  SleepDurationInSpt ~ fhr2 + sex + weekend + s(age, k = 10) + 
    s(age, by = fhr2, k = 10),
  random = list(fid = ~1, subject_id = ~1),
  data = merged,
  method = "REML"
...
merged$SE <- merged$SleepDurationInSpt/as.numeric(merged$SptDuration)

nonlinear_order2 <- gamm(
...
gam.check(nonlinear_order2$gam)

AIC(nonlinear_order$lme, nonlinear_order2$lme)
#BIC(linear, nonlinear_order)

summary(nonlinear_order2$gam)
