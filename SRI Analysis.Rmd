...
SRIoutput <- ggplot() +
  ...
  y = 62,
  label = expression(bold(italic("p"))[BONF.] == 0.039),
  parse = TRUE,
  hjust = 0
) +
  theme_minimal()
...
#B380B3   #8E4A8E
  ...
  geom_ribbon(
    data = subset(bands, age >= 8 & age < 17.9),
    ...
    # ---- Significant: 18.32 → 25 ----
  geom_ribbon(
    data = subset(bands, age >= 17.9 & age <= 21.1),
    aes(ymin = .lower_ci, ymax = .upper_ci),
    fill  = "#8E4A8E",
    alpha = 0.5
  ) +
  ...
  geom_ribbon(
    data = subset(bands, age > 21.1 & age <= 25),
    aes(ymin = .lower_ci, ymax = .upper_ci),
    fill  = "#B380B3",
    alpha = 0.25
  ) +
