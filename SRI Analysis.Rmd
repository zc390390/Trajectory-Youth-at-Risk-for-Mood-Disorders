...
# Wald (fast, uses SEs)
confint(linear, method = "Wald")

# Profile-likelihood (more accurate, slower)
confint(linear, method = "profile")


linear_exploration <- lmer(SRI2_scaled ~ mdd*age + bd*age + psy*age + sex + weekend + (1|fid/subject_id), data = merged)
...
save_as_docx(ft, path = "SRI_APA_Table.docx")
```
...
MIDcombo <- yep + yep2 +
  plot_layout(ncol = 2, widths = c(1, 1)) +
  plot_annotation(
    title = "Predicted SRI & Observed Mean SRI",
    theme = theme(
      plot.title = element_text(size = 18, face = "bold", hjust = 0.5),
      plot.margin = margin(15, 15, 15, 15)
    )
