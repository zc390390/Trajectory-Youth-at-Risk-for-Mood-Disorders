...
merged2 <- merged2 %>% filter(diffdays > 0 & diffdays <= 300)
...
SRIoutput <- ggplot() +
  ...
  y = 62,
  label = expression(bold(italic("p"))[BONF.] > 0.001),
  parse = TRUE,
  hjust = 0
) +
  theme_minimal()
...
#B380B3   #8E4A8E
  ...
  geom_ribbon(
    data = subset(bands, age >= 8 & age < 17.6),
    ...
    # ---- Significant: 18.32 → 25 ----
  geom_ribbon(
    data = subset(bands, age >= 17.6 & age <= 22),
    aes(ymin = .lower_ci, ymax = .upper_ci),
    fill  = "#8E4A8E",
    alpha = 0.5
  ) +
