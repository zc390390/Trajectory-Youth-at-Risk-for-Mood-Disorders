...
linear <- lmer(midpoint ~ fhr2*age + sex + weekend + (1|fid/subject_id), data = merged)
...
MIDdiff <- ggplot(bands, aes(x = age, y = .estimate)) +
  ...
  geom_ribbon(
    data = subset(bands, age >= 8 & age < 10.1),
  ...
  geom_ribbon(
    data = subset(bands, age >= 10.1 & age <= 13.34),
    aes(ymin = .lower_ci, ymax = .upper_ci),
    fill  = "#8E4A8E",
    alpha = 0.5
  ) +
  ...
  geom_ribbon(
    data = subset(bands, age > 13.34 & age <= 17.5),
    aes(ymin = .lower_ci, ymax = .upper_ci),
    fill  = "#B380B3",
    alpha = 0.25
  ) +
