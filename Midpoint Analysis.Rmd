...
##Timing
```{r}
linear1 <- lmer(sleeponset ~ fhr*age + sex + weekend + (1|fid/subject_id), data = merged)
summary(linear1)

p <- (0.0105*4)
print(p)
confint(linear1, method = "profile")


linear1_exploration <- lmer(sleeponset ~ mdd*age + bd*age + psy*age + sex + weekend + (1|fid/subject_id), data = merged)
summary(linear1_exploration)
p_mdd <- (0.00102*4)
p_bd <- (0.5*4)
p_psy <- (0.9*4)
print(paste0("MDD P = ", p_mdd, "  BD P = ", p_bd, "  PSY P =", p_psy))
confint(linear1_exploration, method = "profile")

linear2 <- lmer(wakeup ~ fhr*age + sex + weekend + (1|fid/subject_id), data = merged)
summary(linear2)
confint(linear2, method = "profile")
p <- 'nonsig'

linear2_exploration <- lmer(wakeup ~ mdd*age + bd*age + psy*age + sex + weekend + (1|fid/subject_id), data = merged)
summary(linear2_exploration)
p <- 'nonsig for all'
...
####Onset
```{r}
mod1 <- broom.mixed::tidy(linear1, effects = "fixed", conf.int = TRUE)
mod2 <- broom.mixed::tidy(linear1_exploration, effects = "fixed", conf.int = TRUE)
...
save_as_docx(ft, path = "Onset_APA_Table.docx")
```

####Wakeup
```{r}
mod1 <- broom.mixed::tidy(linear2, effects = "fixed", conf.int = TRUE)
mod2 <- broom.mixed::tidy(linear2_exploration, effects = "fixed", conf.int = TRUE)
...
save_as_docx(ft, path = "Wakeup_APA_Table.docx")
```
###Visualize Linear
####Onset
```{r}
...
newdat$fit <- predict(linear1, newdata = newdat, re.form = NA, allow.new.levels = TRUE)

sum <- merged %>%
  group_by(filename) %>%
    summarise(
      ONSETm = mean(sleeponset, na.rm = TRUE),
...
yep <- ggplot(newdat, aes(x = age, y = fit, color = fhr, fill = fhr)) +
  ...
  scale_y_continuous(
    ...
    labels = c("6:00 PM", "8:00 PM", "10:00 PM",
...
newdat$fit <- predict(linear1_exploration, newdata = newdat, re.form = NA, allow.new.levels = TRUE)
...
yep2 <- ggplot(newdat, aes(x = age, y = fit, color = group)) +
  ...
  geom_point(
    data = sum,
    aes(x = age, y = ONSETm, color = group),
    alpha = 0.15,
    size = 1.2,
    inherit.aes = FALSE
  ) +
...
MIDcombo <- yep + yep2 +
  ...
  plot_annotation(
    title = "Predicted Sleep Onset & Observed Mean Sleep Onset",
    theme = theme(
...
newdat$fit <- predict(linear2, newdata = newdat, re.form = NA, allow.new.levels = TRUE)

sumdat <- merged %>%
  group_by(filename) %>%
    summarise(
      WAKEUPm = mean(wakeup, na.rm = TRUE),
      age = mean(age, na.rm = TRUE),
      fhr = first(fhr)
    )
...
yep <- ggplot(newdat, aes(x = age, y = fit, color = fhr, fill = fhr)) +
  ...
  scale_y_continuous(
    ...
    labels = c("12:00 AM", "2:00 AM", "4:00 AM",
...
newdat$fit <- predict(linear2_exploration, newdata = newdat, re.form = NA, allow.new.levels = TRUE)
...
yep2 <- ggplot(newdat, aes(x = age, y = fit, color = group)) +
  ...
  geom_point(
    data = sumdat,
    aes(x = age, y = WAKEUPm, color = group),
    alpha = 0.15,
    size = 1.2,
    inherit.aes = FALSE
  ) +
...
MIDcombo <- yep + yep2 +
  plot_layout(ncol = 2, widths = c(1, 1)) +
  plot_annotation(
    title = "Predicted Sleep Wakeup & Observed Mean Sleep Wakeup",
    theme = theme(
      plot.title = element_text(size = 18, face = "bold", hjust = 0.5),
      plot.margin = margin(15, 15, 15, 15)
    )
...
nonlinear <- gamm(
  sleep_midpoint_min_adj ~ fhr + sex + weekend + 
...
nonlinear_order <- gamm(
  sleep_midpoint_min_adj ~ fhr + sex + weekend + s(age, k = 5) +
...
sum <- merged %>%
  group_by(filename) %>%
    summarise(
      MIDm = mean(sleep_midpoint_min_adj, na.rm = TRUE)/60,
      age = mean(age, na.rm = TRUE),
      fhr = first(fhr)
    )
...
MIDoutput <- ggplot() +
  ...
  geom_line(data = newdat,
            aes(x = age, y = fit/60, color = fhr),
            size = 1.2) +
  geom_ribbon(data = newdat,
              aes(x = age, ymin = lower/60, ymax = upper/60, fill = fhr),
              alpha = 0.25, color = NA) +
...
ggplot(merged, aes(x = sleep_midpoint_min_adj)) +
  geom_histogram(binwidth = 30, color = "white", fill = "#6699FF") +
  labs(
...
merged %>%
  filter(sleep_midpoint_min_adj < -200) %>%
  select(filename, subject_id, age, fhr, sleep_midpoint_min, sleep_midpoint_min_adj, sleeponset, wakeup) %>%
  arrange(desc(sleep_midpoint_min_adj))




linear <- lmer(sleep_midpoint_min_adj ~ fhr*age + sex + weekend + (1|fid/ subject_id), data = merged)
summary()
```
