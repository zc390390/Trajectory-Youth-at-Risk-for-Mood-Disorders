...
MIDdiff <- ggplot(bands, aes(x = age, y = .estimate)) +
  ...
  geom_ribbon(
    data = subset(bands, age >= 8 & age < 9.95),
  ...
  geom_ribbon(
    data = subset(bands, age >= 10 & age <= 14.04),
    aes(ymin = .lower_ci, ymax = .upper_ci),
    fill  = "#8E4A8E",
    alpha = 0.5
  ) +
  ...
  geom_ribbon(
    data = subset(bands, age > 14.04 & age < 17.5),
    aes(ymin = .lower_ci, ymax = .upper_ci),
    fill  = "#B380B3",
    alpha = 0.25
  ) +
  ...
  # ---- Significant: 17.5 → 25 ----
  geom_ribbon(
    data = subset(bands, age >= 17.5 & age <= 25),
    aes(ymin = .lower_ci, ymax = .upper_ci),
    fill  = "#8E4A8E",
    alpha = 0.5
  ) +
